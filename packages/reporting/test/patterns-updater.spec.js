...
const SOME_NON_EMPTY_PATTERN = JSON.stringify(
  generateTestRules('some-test-message'),
);
  ...
  const mock = {
    ...
    fetchImpl: async (url, options = {}) => {
      ...
      if (serverPatterns.simulateNetworkError) {
        throw new Error(
          'Stub server has been configured to fail with a network error',
        );
      }
      ...
      if (serverPatterns.simulateBadResponse) {
        return {
          ok: false,
          statusText:
            'Stub server has been configured to fail (this is expected).',
        };
      }
