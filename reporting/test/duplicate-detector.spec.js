...
describe('#DuplicateDetector', function () {
  ...
  beforeEach(async function () {
    ...
    persistedHashes = new PersistedHashes({
      storage,
      storageKey: 'dummy-storage-key',
    });
  ...
  it('should detect duplicated messages even keys differ, but ignore non-key fields', async function () {
    ...
    await expectSendNotOK(
      mkMessage({
        payload: {
          q,
          nonKey:
            'This is a different text, but it is a duplicate nevertheless',
        },
        deduplicateBy,
      }),
    );
...
describe('#computeDeduplicationKey', function () {
  ...
  it('should support matching array indices', function () {
    duplicatedPayload(
      {
        x: [{ y: 'part of key' }, { y: 'some text not part of index' }],
      },
      {
        x: [{ y: 'part of key' }, { y: 'another text not part of index' }],
      },
      'x.0.y',
    );
    ...
    nonDuplicatedPayload(
      {
        x: [{ y: 'part of key' }, { y: 'some text part of index' }],
      },
      {
        x: [{ y: 'part of key' }, { y: 'another text part of index' }],
      },
      'x.[].y',
    );
